...
scene = canvas(x=0,y=0,width=1600, height =800,title='String and masses configuration')
...
def plotconfig() :
    ...
    ya = L1*x[0] # L1 s in ( th1 )
    xb = xa+L2*x[4] # L1* cos ( th1 )+L2* cos ( th2 )
    yb = ya+L2*x[1] # L1* s in ( th1 )+L2* sen ( th2 )
    xc = xb+L3*x[5] # L1* cos ( th1 )+L2* cos ( th2 )+L3* cos ( th3 )
    yc = yb-L3*x[2] # L1* s in ( th1 )+L2* sen ( th2 )−L3* sin ( th3 )
    mx = 100.0 # for linear coordinate transformation
    bx = -500.0 # from 0=< x =<10
    my = -100.0 # to −500 =<x_window=>500
    by = 400.0 # same transformat ion for y
    xap = mx*xa+bx # to keep aspect ra t io
    yap = my*ya+by
...
for it in range(1 , 100) :
    ...
    sol = solve( deriv , B)
    dx = sol[:,0]   # F i r s t column o f so l
    for i in range(0 , n) :
        x[ i ] = x[ i ] + dx[ i ]
    plotconfig ()
    errX = errF = errXi = 0.0
